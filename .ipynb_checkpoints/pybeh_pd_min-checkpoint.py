...
def pd_min_crp_subj(df, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type', max_n_reps=1):
    ...
    if min_lag_num != 0:
        ...
        prob = min_crp_subj(recalls=recalls,
...
def min_crp_subj(recalls=None, poss_recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
# subjects=None
        ...
        for j, trial_recs in enumerate(recalls):
            ...
            for k, rec in enumerate(trial_recs[:-1]):
                ...
                if with_repeats == True:
                    rec = rec[rec != 0]
                    seen.update(rec)
                if with_repeats == False:
                    seen.update(rec)
                # Only increment transition counts if the current and next recall are BOTH correct recalls
                if clean_recalls_mask[0][k] and clean_recalls_mask[0][k + 1] and k >= skip_first_n:
                    next_rec = trial_recs[k + 1]
                    next_rec = next_rec[next_rec != 0]
                    if with_repeats:
                        pt = np.unique(np.array([get_min_trans_subj(serialpos[serialpos != 0], rec) for serialpos in poss_recalls if serialpos[0] not in seen]))#don't increment more than once
                    else:
                        pt = np.unique(np.array([get_min_trans_subj(serialpos[serialpos != 0], rec) for serialpos in poss_recalls if serialpos not in seen]))#don't increment more than once

                    pt = np.array([x for x in pt if np.isnan(x) == False], dtype=int)
                    poss[pt + listLength - 1] += 1
                    trans = get_min_trans_subj(next_rec, rec)
...
def pd_min_crp_subj_intr(df, intr_item_num_list, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type', max_n_reps=1):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
      pres_type=pres_type, rec_type=rec_type, type_column=type_column, max_n_reps=max_n_reps)
    poss_recalls = make_poss_recalls_matrix(pres_itemnos=pres_itemnos, max_n_reps=max_n_reps)
    ...
    if min_lag_num != 0:
        ...
        prob = min_crp_subj_intr(intr_item_num_list=intr_item_num_list, recalls=recalls,
                       poss_recalls=poss_recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1],
                    lag_num=lag_num)
...
def min_crp_subj_intr(intr_item_num_list, recalls=None, poss_recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
# subjects=None
...
def pd_mincrp_subj_fixedlags(df, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type', max_n_reps=1):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
      pres_type=pres_type, rec_type=rec_type, type_column=type_column, max_n_reps=max_n_reps)
    poss_recalls = make_poss_recalls_matrix(pres_itemnos=pres_itemnos, max_n_reps=max_n_reps)
    ...
    if len(recalls) == 0:
        return np.nan
    if min_lag_num != 0:
        prob = mincrp_subj_fixedlags(recalls=recalls,
                       poss_recalls=poss_recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1])
...
def mincrp_subj_fixedlags(recalls=None, poss_recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
# subjects=None
...
def get_min_trans_subj(serialpos, rec):
...
def crp_plusminus_one(recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
    '''
    CRP   Conditional response probability as a function of lag (lag-CRP).
      ...
      OUTPUTS:
            lag_crps:  A matrix of lag-CRP values.  Each row contains the
                       value for one subject.  It has one column corresponding 
                       to the crp of lags +1 and -1 combined. If transitions of 
                       lags +1 or -1 were impossible for the input data for 
                       that subject, the value assigned to that subject is NaN. 
        ...
        for j, trial_recs in enumerate(recalls[subjects == subj]):
            ...
            for k, rec in enumerate(trial_recs[:-1]):
                ...
                if clean_recalls_mask[j][k] and clean_recalls_mask[j][k + 1] and k >= skip_first_n:
                    ...
                    trans = next_rec - rec
                    # if the lag is -/+ 1 increment the actual couter
                    if trans == 1 or trans == -1:
                        actual += 1               
        #print(actual, poss)
        if poss!=0:
...
def actual_and_poss(recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
    ...
    actual = np.empty(num_lags)
    poss = np.empty(num_lags)
    actual_array=np.zeros((usub.size, num_lags))
    poss_array=np.zeros((usub.size, num_lags))
    ...
    for i, subj in enumerate(usub):
        ...
        actual_array[i, :] = actual
        poss_array[i, :] = poss

    return actual_array, poss_array



def pd_actual_and_poss(df, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type'):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
    ...
    if len(recalls) == 0:
        return pd.DataFrame()
    if min_lag_num != 0:
        actual, poss = actual_and_poss(recalls=recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1],
                    lag_num=lag_num)
#         poss = actual_and_poss(recalls=recalls,
#                     subjects=['_'] * recalls.shape[0],
#                     listLength=pres_itemnos.shape[1],
#                     lag_num=lag_num)[1]
#         print(actual, poss)
    else:
        actual = np.full((lag_num*2)+1, np.nan)
        poss = np.full((lag_num*2)+1, np.nan)
    crp_dict = {'actual_count': actual[0], 
                'poss_count': poss[0],
                'lag': np.arange(-lag_num, (lag_num+1))}
    return pd.DataFrame(crp_dict, index=np.arange(-lag_num, (lag_num+1)))
...
def pd_crp_plusminus_one(df, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type'):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
      pres_type=pres_type, rec_type=rec_type, type_column=type_column)
    min_lag_num = min(pres_itemnos.shape[1], lag_num)
    ...
    if len(recalls) == 0:
        return np.nan
    if min_lag_num != 0:
        prob = crp_plusminus_one(recalls=recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1])
    else:
        prob = np.nan
#     crp_dict = {'prob': prob, 
#                 'lag': 'small'}
#     return pd.DataFrame(crp_dict, index=1)
    return prob


#MIN CRP FOR BINS

def min_actual_and_poss(recalls=None, poss_recalls=None, subjects=None, listLength=None, lag_num=None, skip_first_n=0):
    if recalls is None:
        raise Exception('You must pass a recalls matrix.')
    elif subjects is None:
        raise Exception('You must pass a subjects vector.')
...
def pd_min_actual_and_poss(df, lag_num=12, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type', max_n_reps=3):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
      pres_type=pres_type, rec_type=rec_type, type_column=type_column, max_n_reps=max_n_reps)
    poss_recalls = make_poss_recalls_matrix(pres_itemnos=pres_itemnos, max_n_reps=max_n_reps)
    ...
    if len(recalls) == 0:
        return pd.DataFrame()
    if min_lag_num != 0:
        actual, poss = min_actual_and_poss(recalls=recalls,
                       poss_recalls=poss_recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1],
                    lag_num=lag_num)
...
def pd_mincrp_fixedlags(df, lag_num=5, itemno_column='itemno', list_index=['subject', 'session', 'list'], pres_type="WORD", rec_type="REC_WORD", type_column='type', max_n_reps=1):
    """Expects as input a dataframe (df) for one subject"""
    pres_itemnos, rec_itemnos, recalls = get_all_matrices(df, itemno_column=itemno_column, list_index=list_index, 
      pres_type=pres_type, rec_type=rec_type, type_column=type_column, max_n_reps=max_n_reps)
    poss_recalls = make_poss_recalls_matrix(pres_itemnos=pres_itemnos, max_n_reps=max_n_reps)
    ...
    if len(recalls) == 0:
        return np.nan
    if min_lag_num != 0:
        prob = mincrp_fixedlags(recalls=recalls,
                       poss_recalls=poss_recalls,
                    subjects=['_'] * recalls.shape[0],
                    listLength=pres_itemnos.shape[1])
...
def mincrp_fixedlags(recalls=None, poss_recalls=None, subjects=None, listLength=None, lag_num=None,skip_first_n=0):
...
def pd_sem_crp_list(df, sim_columns=None, bins=None, pres_type="WORD", 
    ...
    pres_values = pres_df[sim_columns].values
    if pres_values.ndim == 1:
        pres_values = np.array([pres_df[sim_columns].values]).T
    
    sem_sims = distance_matrix(pres_values, pres_values, p=p)
    n_bins = len(bins)
    #print(recalls, rec_itemnos, pres_itemnos)
